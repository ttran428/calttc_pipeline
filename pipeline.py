...
@click.command()
@click.option('--form', default='nt',
              help='nt=Novice Training, at=Advanced training, rr=Round Robin')
@click.option('--input', default='test_form',
              help="the filepath to the csv input file")
@click.option('--output', default='test_form',
              help="the filepath to the csv output file")
def main(form: str, input: str, output: str):
    ...
    if not valid_input(input_df):
        raise ValueError("Make sure form has fields: "
                         "SID, Name, and Email formatted correctly.")
    ...
    if form == 'nt':
        ...
        raise ValueError("Please check type again. Options for form are: "
